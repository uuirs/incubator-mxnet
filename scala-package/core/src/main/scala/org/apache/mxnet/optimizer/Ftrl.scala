 ...
 */
class Ftrl(val learningRate: Float = 0.002f, rescaleGradient: Float = 1.0f,
           lambda1: Float = 0.95f, beta: Float = 0.9f, wd: Float = 0.0f,
...
*/
    ...
    val (z, n) = state.asInstanceOf[(NDArray, NDArray)]
    NDArray.api.ftrl_update(weight,grad,z,n,learningRate,Some(lambda1),Some(beta),Some(wd),Some(rescaleGradient),Some(clipGradient))

  }

  override def createState(index: Int, weight: NDArray): (NDArray, NDArray) = {
    (NDArray.zeros(weight.shape, weight.context), // z
  ...
  override def serializeState(state: AnyRef): Array[Byte] = {
    if (state != null) {
      val (z, n) = state.asInstanceOf[(NDArray, NDArray)]
      SerializerUtils.serializeNDArrays(z, n)
    } else {
      null
    }
  }
