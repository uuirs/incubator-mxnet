    ...
    try {
      ...
      for (i <- 0 until numNdarray) {
        if (tags(i) == 1 || tags(i) == 4) {
          tensors(tags(i)) += NDArray.cls(ndarraies(i), writable = true, addToCollector = false)
        } else {
          tensors(tags(i)) += NDArray.cls(ndarraies(i), writable = false, addToCollector = false)
        }
      }
    ...
    try {
      ...
      for (i <- 0 until numNdarray) {
        if (tags(i) == 2 || tags(i) == 4) {
          tensors(tags(i)) += NDArray.cls(ndarraies(i), writable = true)
        } else {
          tensors(tags(i)) += NDArray.cls(ndarraies(i), writable = false)
        }
      }
      val reqEnum = Array("null", "write", "inplace", "add")
